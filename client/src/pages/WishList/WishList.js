...
function WishList() {
  ...
  const removeFromWishlist = async (productId) => {
    try {
      const response = await axios.post(`http://localhost:5000/wishlist/remove`, {
        userId: user?.userId, // Ensure `userId` is available
        productId,
      });

      if (response.data.success) {
        const updatedWishlist = wishlistProducts.filter((p) => p._id !== productId);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {wishlistProducts.map((product) => (
            <div key={product._id} className="border rounded-lg p-4 shadow-md relative">
